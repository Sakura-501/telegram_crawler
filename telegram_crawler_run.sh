...
while [ 1 ]; do
    ...
    python3 ${PWD}/main.py keywords
    sleep 5;
    echo "2.1 从数据库获取关键字（默认99个），去查询group/channel"
    python3 ${PWD}/main.py search_group_channel from_table
    sleep 5;
    echo "2.2 从config.ini获取关键字（需自行添加），去查询group/channel"
    python3 ${PWD}/main.py search_group_channel from_config
    sleep 5;
    echo "3. 从数据库获取group/channel，查询历史消息（默认200个，每个1000条）"
    nohup python3 ${PWD}/main.py history_message > telegram_crawler.log 2>&1 &
    # 12个小时
    sleep 43200;
    echo "4. 从数据库获取历史消息，过滤出外链（默认20w条）"
    python3 ${PWD}/main.py external_links
    sleep 5;
done